...
export class StageGateStore {
  ...
  transition(versionId,key,to){const s=this.get(versionId,key);if(!s)throw new Error("未知阶段");if(!ALLOWED[s.status]?.includes(to))throw new Error("非法阶段迁移 "+s.status+" -> "+to);this.db.prepare("UPDATE stages SET status=?,completed_at=? WHERE id=?").run(to,to==="approved"?new Date().toISOString():null,s.id);this.events?.append("stage."+to,{versionId,stage:key});return this.get(versionId,key);}
