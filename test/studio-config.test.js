...
test("loads composable role capabilities",()=>{const c=loadStudioConfig("./examples/studio-small/agents.json");assert.ok(c.agents.find(a=>a.id==="manager").capabilities.includes("process-monitoring"));const large=loadStudioConfig("./examples/studio-large/agents.json");assert.ok(large.agents.find(a=>a.id==="control").capabilities.includes("global-monitoring"));});
test("validates capabilities and delegation arrays",()=>{const result=validateStudioConfig({agents:[{id:"x",role:"a",capabilities:"bad",delegatesTo:{}}],providers:{mock:{kind:"mock"}}});assert.equal(result.ok,false);assert.equal(result.errors.length,2);});
