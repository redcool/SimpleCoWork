...
test("rejects invalid four-part version",()=>{const root=mkdtempSync(join(tmpdir(),"studio-cowork-"));const db=openStudioDb(join(root,".cowork","project.db"));const s=new StudioVersionStore({db,projectRoot:root});const p=s.createProject({name:"demo"});assert.throws(()=>s.createVersion({projectId:p.id,version:"0.1.0"}),/四段格式/);closeStudioDb(db);});
